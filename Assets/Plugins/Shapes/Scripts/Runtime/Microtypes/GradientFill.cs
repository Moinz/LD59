...
namespace Shapes
{
    ...
    public struct GradientFill
    {
        ...
        public static readonly GradientFill defaultFill = new()
        {
        ...
        public static GradientFill Linear(Vector3 start, Vector3 end, Color colorStart, Color colorEnd,
            FillSpace space = FillSpace.Local)
        {
        ...
        /// <param name="colorOuter">
        ///     The color at the outer part of the gradient, at <c>radius</c> distance away from the
        ///     <c>origin</c>
        /// </param>
        /// <param name="space">Whether or not this gradient should be in local or world space</param>
        public static GradientFill Radial(Vector3 origin, float radius, Color colorInner, Color colorOuter,
            FillSpace space = FillSpace.Local)
        {
            return new GradientFill
            {
        ...
        {
            return use ? (int)type : FILL_NONE;
        }

        // used for editor stuff only
#if UNITY_EDITOR
        private bool WorldSpace => space == FillSpace.World;

        public Vector3 GetRadialOriginWorld(Transform tf)
        {
            return GetWorldPos(tf, radialOrigin);
        }

        public void SetRadialOriginWorld(Transform tf, Vector3 worldOrigin)
        {
            SetWorldPos(tf, ref radialOrigin, worldOrigin);
        }

        public Vector3 GetLinearStartWorld(Transform tf)
        {
            return GetWorldPos(tf, linearStart);
        }

        public void SetLinearStartWorld(Transform tf, Vector3 worldOrigin)
        {
            SetWorldPos(tf, ref linearStart, worldOrigin);
        }

        public Vector3 GetLinearEndWorld(Transform tf)
        {
            return GetWorldPos(tf, linearEnd);
        }

        public void SetLinearEndWorld(Transform tf, Vector3 worldOrigin)
        {
            SetWorldPos(tf, ref linearEnd, worldOrigin);
        }

        private Vector3 GetWorldPos(Transform tf, Vector3 coord)
        {
            return WorldSpace ? coord : tf.TransformPoint(coord);
        }

        private void SetWorldPos(Transform tf, ref Vector3 field, Vector3 worldValue)
        {
            field = WorldSpace ? worldValue : tf.InverseTransformPoint(worldValue);
        }

        public float GetRadialWorldRadius(Transform tf)
        {
            return WorldSpace ? radialRadius : tf.lossyScale.x * radialRadius;
        }
        ...
        public static GradientFill CreateLinear(Vector3 start, Vector3 end, Color colorStart, Color colorEnd,
            FillSpace space)
        {
            return default;
        }
        ...
        public static GradientFill CreateRadial(Vector3 origin, float radius, Color colorInner, Color colorOuter,
            FillSpace space)
        {
            return default;
        }
